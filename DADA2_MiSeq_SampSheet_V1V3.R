...
map_file <- read.csv(paste0(out_path, "/", mapping_file), skip=19,row.names=1)
...
if(!exists("initial_step")) {
 ...
 fnFs[[i]] <- list.files(paste0(out_path, "/adapter_filtered"), pattern="_R1", full.names=TRUE)
 fnRs[[i]] <- list.files(paste0(out_path, "/adapter_filtered"), pattern="_R2", full.names=TRUE)
 filt_path <- file.path(save_path, "filtered")
 sample.names <- gsub("_R1_001.fastq.gz","",basename(fnFs[[i]]))
...
out <- filterAndTrim(fwd=unlist(fnFs[[i]]), filt=unlist(filtFs[[i]]), rev=unlist(fnRs[[i]]), filt.rev=unlist(filtRs[[i]]), 
              maxEE=c(2,5), rm.phix=TRUE, truncLen=c(280,240),
              compress=TRUE, verbose=TRUE, matchIDs = TRUE, multithread=threads)
...
keep.names <- gsub("R1_001.fastq.gz", "", names(keep[keep])) ## Removing the underscore to denote the end of the sample name in those that get dropped (for instance if S3 gets dropped, but there's also an S30, it would get dropped previously) This should mitigate that.
...
mergers <- mergePairs(flatten(dadaFs), unlist(filtFs), flatten(dadaRs), unlist(filtRs), minOverlap=15, verbose=TRUE)
...
orig <- seqtab.nochim
rownames(orig) <- sub("_S.*_R1_filt.fastq.gz","", rownames(orig))
